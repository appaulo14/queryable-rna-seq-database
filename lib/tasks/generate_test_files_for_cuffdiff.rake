namespace :generate_test_files do
  ...
  task :cuffdiff => :environment do
    ...
    if not Dir.exists?("#{Rails.root}/tmp/generated_test_files/cuffdiff")
      Dir.mkdir("#{Rails.root}/tmp/generated_test_files/cuffdiff")
    end
    if Dir.exists?("#{Rails.root}/tmp/generated_test_files/cuffdiff/regular")
      system("rm -r #{Rails.root}/tmp/generated_test_files/cuffdiff/regular")
    end
    Dir.mkdir("#{Rails.root}/tmp/generated_test_files/cuffdiff/regular")
    Dir.chdir("#{Rails.root}/tmp/generated_test_files/cuffdiff/regular")
    GenerateCuffdiffFiles.make_samples(3)
    GenerateCuffdiffFiles.make_genes(1000)
    ...
    if Dir.exists?("#{Rails.root}/tmp/generated_test_files/cuffdiff/large")
      system("rm -r #{Rails.root}/tmp/generated_test_files/cuffdiff/large")
    end
    Dir.mkdir("#{Rails.root}/tmp/generated_test_files/cuffdiff/large")
    Dir.chdir("#{Rails.root}/tmp/generated_test_files/cuffdiff/large")
    GenerateCuffdiffFiles.make_samples(4)
    GenerateCuffdiffFiles.make_genes(60000)
    GenerateCuffdiffFiles.make_transcripts()
    GenerateCuffdiffFiles.make_transcripts_fasta_file()
    GenerateCuffdiffFiles.make_isoform_fpkm_file()
    GenerateCuffdiffFiles.make_isoform_det_files()
